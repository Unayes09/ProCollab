  ...
  const htmlContent = `
        ...
        <body>
            <div class="container">
                ...
                <h5>Please go back to Log In.</h5>
  ...
  try {
    ...
    if (!userRes) {
      res
        .status(404)
        .send(
          'User not found.Please go to <a href="http://localhost:5173/signin">Link</a>'
        );
    }
...
exports.allChannels = async (req, res) => {
  try {
    if (req.query.id) {
      ...
      const project = await ChannelModel.find({
        channel_holder: req.query.user,
      });
      if (!project) {
        res.status(404).json("Channel not found");
      } else res.status(200).json(project);
